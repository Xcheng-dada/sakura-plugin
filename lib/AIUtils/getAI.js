...
// 从 429 错误中提取官方建议的等待时长（retryDelay），解析失败则使用指数退避
function get429RetryDelay(err, attempt) {
  try {
    ...
    for (const detail of details) {
      ...
      if (typeof retryDelay === "string" && retryDelay.endsWith("s")) {
        ...
        if (!isNaN(seconds) && seconds > 0) {
          return Math.min((seconds + 1) * 1000, 120000)
        }
      }
    }
  } catch {}
...
async function _getGeminiResponse(
  ...
  try {
    ...
    const maxRetries = Math.max(0, Number(channel.retryAttempts ?? 5))
    let retryCount = 0
    let response
    while (true) {
      try {
        response = await ai.models.generateContent(geminiRequestBody)
        break
      } catch (err) {
        ...
        retryCount++
        const delay = get429RetryDelay(err, retryCount)
        logger.warn(
          `Gemini 429 限流，${Math.round(delay / 1000)} 秒后重试（第 ${retryCount}/${maxRetries} 次）`,
        )
