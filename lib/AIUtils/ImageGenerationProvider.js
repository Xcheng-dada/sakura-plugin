...
const DASHSCOPE_PROVIDER_NAMES = new Set(["dashscope", "bailian", "qwen", "qwen-image", "qwen_image", "aliyun", "alibaba"])
const DEFAULT_OPENAI_BASE_URL = "https://api.openai.com/v1"
const DEFAULT_DASHSCOPE_GENERATION_URL =
...
export function getImageProvider(imageConfig = {}) {
  ...
  if (DASHSCOPE_PROVIDER_NAMES.has(provider)) return "dashscope"
  if (OPENAI_PROVIDER_NAMES.has(provider)) return "openai"
  return "gemini"
}
...
function resolveDashScopeImageSize({ aspectRatio, imageConfig }) {
  const configuredSize = imageConfig.openaiSize || imageConfig.openai_size || imageConfig.size
  if (configuredSize) return String(configuredSize).replace(/x/i, "*")
  ...
  if (ratio.width >= ratio.height) {
    width = base
    height = Math.round((base * ratio.height) / ratio.width / 16) * 16
  } else {
    height = base
    width = Math.round((base * ratio.width) / ratio.height / 16) * 16
  }
...
function extractDashScopeResult(response) {
  const content = response?.output?.choices?.[0]?.message?.content
  const imageItem = Array.isArray(content) ? content.find(item => item && item.image) : null
  if (imageItem) {
    return { imageUrl: imageItem.image, raw: response }
  }
  return { text: response?.message || "API 未返回图片。", raw: response }
}

async function generateDashScopeImage({ imageConfig, prompt, imageInputs, aspectRatio, imageSize }) {
  if (!imageConfig || !imageConfig.api || !imageConfig.model) {
    throw new Error("配置错误：未在 'EditImage' 配置中找到有效的百炼配置或缺少 api/model。")
  }
  ...
  const content = imageInputs.map(imageInput => ({
    image: `data:${imageInput.mimeType};base64,${imageInput.base64}`,
  }))
  content.push({ text: prompt })

  const parameters = {}
  const n = Number.parseInt(imageConfig.n || 1, 10)
  parameters.n = Number.isFinite(n) && n > 0 ? n : 1

  const size = resolveDashScopeImageSize({ aspectRatio, imageConfig })
  if (size) parameters.size = size

  const negativePrompt = imageConfig.negativePrompt ?? imageConfig.negative_prompt
  if (negativePrompt !== undefined && negativePrompt !== null && negativePrompt !== "") {
    parameters.negative_prompt = negativePrompt
  }

  if (imageConfig.promptExtend !== undefined && imageConfig.promptExtend !== null) {
    parameters.prompt_extend = Boolean(imageConfig.promptExtend)
  }

  if (imageConfig.watermark !== undefined && imageConfig.watermark !== null) {
    parameters.watermark = Boolean(imageConfig.watermark)
  }

  const endpoint =
    imageConfig.baseURL && imageConfig.baseURL.includes("services/aigc")
      ? imageConfig.baseURL
  ...
  try {
    ...
    throw new Error(`请求百炼图片生成 API 失败: ${error.message}`)
  }
  ...
  if (!response.ok) {
    throw new Error(
      `百炼图片生成接口返回 ${response.status}: ${data.message || data.code || "未知错误"}`,
    )
  }

  const result = extractDashScopeResult(data)
  if (!result.imageUrl) {
    throw new Error(result.text || "百炼接口未返回图片。")
  }

  return { provider: "dashscope", ...result }
}
...
export async function generateImageWithConfig({
  ...
  if (provider === "dashscope") {
    return generateDashScopeImage({ imageConfig, prompt, imageInputs, aspectRatio, imageSize })
  }
